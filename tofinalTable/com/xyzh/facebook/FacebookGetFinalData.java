 ...
 *
 */
public class FacebookGetFinalData {
	 ...
	 * @throws SQLException
	 */
	public void facebookToFinalTotal(String dateTime_str)
		...
		boolean flag = tableWhetherExist.whetherExist("t_facebook_total_"
		...
		if (flag) {
			String sql = "select  c.Id as columnId,c.parentId as parentId,c.cnName as title,c.level as level, "
					...
					+ "FROM  t_facebook_total_"+dateTime_tableName+"   a  "
			...
			System.out
					.println("开始插入+++++++++++++++++++++++++++++++++++++++++++++++++++++");
			odb.addToProductBase(resultList, "t_facebook_total_final");
			System.out
					.println("插入完成+++++++++++++++++++++++++++++++++++++++++++++++++++++");
		} else {
			System.out.println("t_facebook_total_" + dateTime_tableName
					+ "表不存在");
		}
	}

	/**
	 * facebook dayData and interval
	 * 
	 * @param dateTime_str
	 * @throws SQLException
	 * @throws ParseException
	 */
	public void facebookToFinalDay(String dateTime_str) throws SQLException,
			ParseException {
		OperateDatabase odb = new OperateDatabase();
		String dateTime_tableName = TimeUtil.getTableName(dateTime_str);
		String dateTime_tableName2 = TimeUtil.getTableName2(dateTime_str);
		boolean flag = new TableWhetherExist().whetherExist("t_facebook_day_"
				+ dateTime_tableName, "baseForReptile");
		if (flag) {
			boolean flag2 = new TableWhetherExist().whetherExist(
					"t_facebook_day_" + dateTime_tableName2, "baseForReptile");
			if (!flag2) {
				while (!flag2) {
					System.out.println("t_facebook_day_" + dateTime_tableName2
							+ "表不存在，往前推一天比较增量");
					...
					flag2 = new TableWhetherExist().whetherExist(
							"t_facebook_day_" + dateTime_tableName2,
							"baseForReptile");
				}
			}
			int week = TimeUtil.getWeekOfYear(dateTime_str);
			...
			String sql = "select  c.Id as columnId,c.parentId as parentId,c.cnName as title,c.level as level,"
					...
					+ "a.dateTime as dateTime,a.languageType as language  "
					+ "FROM  t_facebook_day_"+dateTime_tableName+"   a  "
					+ "JOIN t_facebook_day_"+dateTime_tableName2+"  d ON a.keyWordId = d.keyWordId  "
			...
			System.out
					.println("开始插入+++++++++++++++++++++++++++++++++++++++++++++++++++++");
			odb.addToProductBase(resultList, "t_facebook_day_final");
			System.out
					.println("插入完成+++++++++++++++++++++++++++++++++++++++++++++++++++++");
		} else {
			System.out.println("t_facebook_day_" + dateTime_tableName + "表不存在");
		}
	}
	 ...
	 * @throws SQLException
	 */
	public void facebookToFinalWeek(String dateTime) throws ParseException,
			SQLException {
		OperateDatabase odb = new OperateDatabase();
		// 周
		...
		int week1HaveDay = new TableWhetherExist().getRecordNumber(
				" t_facebook_day_final ", " and weekNumber = " + (week-1));
		int week2HaveDay = new TableWhetherExist().getRecordNumber(
				" t_facebook_day_final ", " and weekNumber = " + (week-2));
//		int week1HaveDay = new TableWhetherExist().getRecordNumber(
...
//				" t_ggplus_day_final ", " and weekNumber = 52");
		if (weekDay == 1) {
			String sql = "select a.columnId,a.parentId,a.title,a.level, a.dateTime, a.language,"
					...
					+ " from t_facebook_day_final where weekNumber="+(week-1)+" group by title) as  b,  "
					...
					+ " from t_facebook_day_final where weekNumber="+(week-2)+" group by title) as a  "
			...
			odb.addToProductBase(resultList, "t_facebook_week_final_copy");
		} else {
	 ...
	 * @throws SQLException
	 */
	public void facebookToFinalMonth(String dateTime) throws ParseException,
			SQLException {
		OperateDatabase odb = new OperateDatabase();
		// 月
		...
		int monthHaveDay1 = new TableWhetherExist().getRecordNumber(
				" t_facebook_day_final ", " and monthNumber = " + (month-1));
		int monthHaveDay2 = new TableWhetherExist().getRecordNumber(
				" t_facebook_day_final ", " and monthNumber = " + (month-2));
		...
		if (monthDay == 1) {
			String sql = "select a.columnId,a.parentId,a.title,a.level, a.dateTime, a.language,a.monthNumber,"
					...
					+ " from t_facebook_day_final where monthNumber="+(month-1)+" group by title) as  b,  "
					...
					+ " from t_facebook_day_final where monthNumber="+(month-2)+" group by title) as a  "
					+ "where b.title=a.title ";
			DBLinkPoll.setFileName(propertiesName);
			List<Map<String, Object>> resultList = new ArrayList<Map<String, Object>>();
			System.out.println(sql);
			...
			System.out.println(resultList.size());
			odb.addToProductBase(resultList, "t_facebook_month_final");
		}else {
	...
	public static void main(String[] args) throws ParseException, SQLException {
		new FacebookGetFinalData().facebookToFinalDay("2016-2-25");

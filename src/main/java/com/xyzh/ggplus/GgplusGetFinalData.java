 ...
 *
 */
public class GgplusGetFinalData {
	 ...
	 * @throws SQLException
	 */
	public void ggplusToFinalTotal(String dateTime_str) throws ParseException,
			SQLException {
		...
		boolean flag = tableWhetherExist.whetherExist("t_ggplus_total_"
		...
		if (flag) {
			String sql = "select  c.Id as columnId,c.parentId as parentId,c.cnName as title,c.level as level, "
					...
					+ "FROM  t_ggplus_total_"+dateTime_tableName+"   a  "
			...
			System.out
					.println("开始插入+++++++++++++++++++++++++++++++++++++++++++++++++++++");
			odb.addToProductBase(resultList, "t_ggplus_total_final");
			System.out
					.println("插入完成+++++++++++++++++++++++++++++++++++++++++++++++++++++");
		} else {
			System.out.println("t_ggplus_total_" + dateTime_tableName + "表不存在");
		}
	}

	/**
	 * ggplus day data and interval
	 * 
	 * @param dateTime_str
	 * @throws SQLException
	 * @throws ParseException
	 */
	public void ggplusToFinalDay(String dateTime_str) throws SQLException,
			ParseException {
		OperateDatabase odb = new OperateDatabase();
		String dateTime_tableName = TimeUtil.getTableName(dateTime_str);
		...
		if (flag) {
			...
			String sql = "select  c.Id as columnId,c.parentId as parentId,c.cnName as title,c.level as level,"
					...
					+ "a.dateTime as dateTime,a.languageType as language  "
					+ "FROM  t_ggplus_day_"+dateTime_tableName+"   a  "
					+ "JOIN t_ggplus_day_"+dateTime_tableName2+"  d ON a.keyWordId = d.keyWordId  "
			...
			System.out
					.println("开始插入+++++++++++++++++++++++++++++++++++++++++++++++++++++");
			odb.addToProductBase(resultList, "t_ggplus_day_final");
			System.out
					.println("插入完成+++++++++++++++++++++++++++++++++++++++++++++++++++++");
		} else {
	 ...
	 * @throws SQLException
	 */
	public void ggplusToFinalWeek(String dateTime) throws ParseException,
			SQLException {
		OperateDatabase odb = new OperateDatabase();
		// 周
		...
		int week1HaveDay = new TableWhetherExist().getRecordNumber(
				" t_ggplus_day_final ", " and weekNumber = " + (week-1));
		int week2HaveDay = new TableWhetherExist().getRecordNumber(
				" t_ggplus_day_final ", " and weekNumber = " + (week-2));
//		int week1HaveDay = new TableWhetherExist().getRecordNumber(
...
//				" t_ggplus_day_final ", " and weekNumber = 52");
		if (weekDay == 1) {
			String sql = "select a.columnId,a.parentId,a.title,a.level, a.dateTime, a.language,"
					...
					+ " from t_ggplus_day_final where weekNumber="+(week-1)+" group by title) as  b,  "
					...
					+ " from t_ggplus_day_final where weekNumber="+(week-2)+" group by title) as a  "
			...
			odb.addToProductBase(resultList, "t_ggplus_week_final_copy");
		} else {
	 ...
	 * @throws SQLException
	 */
	public void ggplusToFinalMonth(String dateTime) throws ParseException,
			SQLException {
		OperateDatabase odb = new OperateDatabase();
		// 月
		...
		int monthHaveDay1 = new TableWhetherExist().getRecordNumber(
				" t_ggplus_day_final ", " and monthNumber = " + (month-1));
		int monthHaveDay2 = new TableWhetherExist().getRecordNumber(
				" t_ggplus_day_final ", " and monthNumber = " + (month-2));
		...
		if (monthDay == 1) {
			String sql = "select a.columnId,a.parentId,a.title,a.level, a.dateTime, a.language,a.monthNumber,"
					...
					+ " from t_ggplus_day_final where monthNumber="+(month-1)+" group by title) as  b,  "
					...
					+ " from t_ggplus_day_final where monthNumber="+(month-2)+" group by title) as a  "
					+ "where b.title=a.title ";
			DBLinkPoll.setFileName(propertiesName);
			List<Map<String, Object>> resultList = new ArrayList<Map<String, Object>>();
			System.out.println(sql);
			...
			System.out.println(resultList.size());
			odb.addToProductBase(resultList, "t_ggplus_month_final");
		}else {

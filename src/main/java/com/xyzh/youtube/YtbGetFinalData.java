...
import com.xyzh.testLog.TestLog4j;
...
public class YtbGetFinalData {
	 ...
	 * @throws SQLException
	 */
	public void ytbToFinalTotal(String dateTime_str) throws ParseException,
			SQLException {
		...
		boolean flag = tableWhetherExist.whetherExist("t_youtube_total_"
		...
		if (flag) {
			...
			String sql = "select  c.Id as columnId,c.parentId as parentId,c.cnName as title,c.level as level, "
					...
					+ "FROM  t_youtube_total_"+dateTime_tableName+"   a  "
			...
			System.out
					.println("开始插入+++++++++++++++++++++++++++++++++++++++++++++++++++++");
			odb.addToProductBase(resultList, "t_ytb_total_final");
			System.out
					.println("插入完成+++++++++++++++++++++++++++++++++++++++++++++++++++++");
		} else {
			System.out
					.println("t_youbube_total_" + dateTime_tableName + "表不存在");
		}
	}

	/**
	 * 处理ytb日榜数据 YouTube day data and interval
	 * 
	 * @param dateTime_str
	 * @throws SQLException
	 * @throws ParseException
	 */
	public void ytbToFinalDay(String dateTime_str) throws SQLException,
			ParseException {
		OperateDatabase odb = new OperateDatabase();
		String dateTime_tableName = TimeUtil.getTableName(dateTime_str);
		...
		int month = TimeUtil.getMonthOfYear(dateTime_str);
		DBLinkPoll.setFileName(propertiesName);
		boolean flag = new TableWhetherExist().whetherExist("t_youtube_day_"
				+ dateTime_tableName, "baseForReptile");
		if (flag) {
			boolean flag2 = new TableWhetherExist().whetherExist(
					"t_youtube_day_" + dateTime_tableName2, "baseForReptile");
			if (!flag2) {
				while (!flag2) {
					...
					flag2 = new TableWhetherExist().whetherExist(
							"t_youtube_day_" + dateTime_tableName2,
							"baseForReptile");
				}
			}
			List<Map<String, Object>> resultList = new ArrayList<Map<String, Object>>();
//			String sql ="select k.columnId as columnId,c.parentId as parentId,c.cnName as title,c.level as level,"
//					+ "sum(a.contentCount) as contentCount,"
//					+ "ifnull((a.contentCount-ifnull(sum(d.contentCount),0))/sum(d.contentCount),0)*100 as contentCountInterval,"
//					+ "sum(a.viewCount) as viewCount,"
//					+ "ifnull((a.viewCount-ifnull(d.viewCount,0))/d.viewCount,0)*100 as viewCountInterval,"
//					+ "a.goodCount as goodCount,"
//					+ "ifnull((a.goodCount-ifnull(d.goodCount,0))/d.goodCount,0)*100  as goodCountInterval,"
//					+ "a.badCount as badCount,"
//					+ "ifnull((a.badCount-ifnull(d.badCount,0))/d.badCount,0)*100 as badCountInterval,"
//					+ "a.shareCount as shareCount,"
//					+ "ifnull((a.shareCount-ifnull(d.shareCount,0))/d.shareCount,0)*100 as shareCountInterval,"
//					+ "a.commentCount as commentCount,"
//					+ "ifnull((a.commentCount-ifnull(d.commentCount,0))/d.commentCount,0)*100 as commentCountInterval,"
//					+ "a.dateTime as dateTime,a.languageType as language "
//					+ "FROM platformbackstage.t_oper_columns c  "
//					+ "JOIN  t_oper_keywords k ON k.columnId = c.id JOIN t_youtube_day_"
//					+ dateTime_tableName
//					+ " a ON k.name = a.keyWordName "
//					+ "JOIN t_youtube_day_"
//					+ dateTime_tableName2
//					+ " d on a.keyWordName = d.keyWordName";
			String sql = "select  c.Id as columnId,c.parentId as parentId,c.cnName as title,c.level as level,"
					+ "sum(a.contentCount) as contentCount,"
					...
					+ "a.dateTime as dateTime,a.languageType as language  "
					+ "FROM  t_youtube_day_"+dateTime_tableName+"   a  "
					+ "JOIN t_youtube_day_"+dateTime_tableName2+"  d ON a.keyWordId = d.keyWordId  "
			...
			System.out
					.println("开始插入+++++++++++++++++++++++++++++++++++++++++++++++++++++");
			odb.addToProductBase(resultList, "t_ytb_day_final");
			System.out
					.println("插入完成+++++++++++++++++++++++++++++++++++++++++++++++++++++");
		} else {
			System.out.println("t_ytb_day_" + dateTime_tableName + "表不存在");
		}
	}
	 ...
	 * @throws SQLException
	 */
	public void ytbToFinalWeek(String dateTime) throws ParseException,
			SQLException {
		OperateDatabase odb = new OperateDatabase();
		// 周
		...
		int week1HaveDay = new TableWhetherExist().getRecordNumber(
				" t_ytb_day_final ", " and weekNumber = " + (week-1));
		int week2HaveDay = new TableWhetherExist().getRecordNumber(
				" t_ytb_day_final ", " and weekNumber = " + (week-2));
//		int week1HaveDay = new TableWhetherExist().getRecordNumber(
...
//				" t_ggplus_day_final ", " and weekNumber = 52");
		if (weekDay == 1) {
			String sql = "select a.columnId,a.parentId,a.title,a.level, a.dateTime, a.language,"
					...
					+ " from t_ytb_day_final where weekNumber="+(week-1)+" group by title) as  b,  "
					...
					+ " from t_ytb_day_final where weekNumber="+(week-2)+" group by title) as a  "
			...
			odb.addToProductBase(resultList, "t_ytb_week_final_copy");
		} else {
	 ...
	 * @throws SQLException
	 */
	public void ytbToFinalMonth(String dateTime) throws ParseException,
			SQLException {
		OperateDatabase odb = new OperateDatabase();
		// 月
		...
		int monthHaveDay1 = new TableWhetherExist().getRecordNumber(
				" t_ytb_day_final ", " and monthNumber = " + (month-1));
		int monthHaveDay2 = new TableWhetherExist().getRecordNumber(
				" t_ytb_day_final ", " and monthNumber = " + (month-2));
		...
		if (monthDay == 1) {
			String sql = "select a.columnId,a.parentId,a.title,a.level, a.dateTime, a.language,a.monthNumber,"
					...
					+ " from t_ytb_day_final where monthNumber="+(month-1)+" group by title) as  b,  "
					...
					+ " from t_ytb_day_final where monthNumber="+(month-2)+" group by title) as a  "
					+ "where b.title=a.title ";
			DBLinkPoll.setFileName(propertiesName);
			List<Map<String, Object>> resultList = new ArrayList<Map<String, Object>>();
			System.out.println(sql);
			...
			System.out.println(resultList.size());
			odb.addToProductBase(resultList, "t_ytb_month_final");
		}else {
